...
export default function App() {
  ...
  const [appSettings, setAppSettings] = useState<AppSettings>({ title: 'GESTION DE SELLOS CNCH', logo: null, sealTypes: ['Botella', 'Cable', 'Plástico', 'Metálico'], themeColor: '#003594' });
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      const db = await ApiService.getBackup();
      if (db) {
        if (db.settings) {
